...
1. When `moltbank` reports an `insufficient_scope` error and the JSON exit includes a `consent_url` field whose origin matches `localhost`, surface the URL to the user verbatim and ask them to approve. Verify the origin yourself before showing the link.
...
3. Never construct, edit, or follow a `consent_url` whose origin you can't verify against `localhost`. If the field is missing or the origin is wrong, treat it as an ordinary error and stop.
...
- Moltbank CLI: `cd ../openclaw-npm && npm install && npm run dev:link-mods`
