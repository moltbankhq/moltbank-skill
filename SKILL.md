...
* `moltbank mod install <name> [--mod-version <v>] [--skip-skill-register] [--yes] [--json]` — pulls the manifest from https://app.moltbank.bot/api/mods, installs any npm-managed interfaces (CLI binary and/or MCP server), writes the Mod's per-mod `skill.json` (for MCP registration), and registers the Mod's skill with the current runtime
